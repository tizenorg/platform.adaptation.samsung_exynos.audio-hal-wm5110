...
static void __dump_use_case(const char *verb, const char *devices[], int dev_count, const char *modifiers[], int mod_count)
{
    int i, len;
    int written = 0;
    char dump[DUMP_STR_LEN] = { 0, };
    char *p = &dump[0];

    /* Verb */
    len = snprintf(p, DUMP_STR_LEN - written, "Verb [ %s ] Devices [ ", verb ? verb : AUDIO_USE_CASE_VERB_INACTIVE);
    p = dump + (written += (len > 0)? len : 0);

    /* Devices */
    if (devices) {
        for (i = 0; i < dev_count; i++) {
            len = snprintf(p, DUMP_STR_LEN - written, (i != dev_count - 1)? "%s, " : "%s", devices[i]);
            p = dump + (written += (len > 0)? len : 0);
        }
    }
    len = snprintf(p, DUMP_STR_LEN - written, " ] Modifier [ ");
    p = dump + (written += (len > 0)? len : 0);

    /* Modifiers */
    if (modifiers) {
        for (i = 0; i < mod_count; i++) {
            len = snprintf(p, DUMP_STR_LEN - written, (i != mod_count - 1)? "%s, " : "%s", modifiers[i]);
            p = dump + (written += (len > 0)? len : 0);
        }
    }
    len = snprintf(p, DUMP_STR_LEN - written, " ]");
    p = dump + (written += (len > 0)? len : 0);

    *p = '\0';

    AUDIO_LOG_INFO(">>> UCM %s", dump);
}
...
audio_return_t _audio_ucm_set_use_case(audio_hal_t *ah, const char *verb, const char *devices[], const char *modifiers[])
{
    ...
    __dump_use_case(old_verb, old_dev_list, old_dev_count, old_mod_list, old_mod_count);
    ...
    __dump_use_case(verb, devices, dev_count, modifiers, mod_count);
...
exit:
    ...
    if (is_verb_changed == 1 || is_dev_changed == 1 || is_mod_changed == 1) {
        ...
        __dump_use_case(new_verb, new_dev_list, new_dev_count, new_mod_list, new_mod_count);
...
audio_return_t _audio_ucm_set_devices(audio_hal_t *ah, const char *verb, const char *devices[])
{
    ...
    snd_use_case_get(ah->ucm.uc_mgr, "_verb", &old_verb);
    old_dev_count = snd_use_case_get_list(ah->ucm.uc_mgr, "_enadevs", &old_dev_list);
    __dump_use_case(old_verb, old_dev_list, old_dev_count, NULL, 0);

    if (devices) {
        for (dev_count = 0; devices[dev_count]; dev_count++);
    }

    __dump_use_case(verb, devices, dev_count, NULL, 0);
...
exit:
    ...
    if (is_verb_changed == 1 || is_dev_changed == 1) {
        ...
        snd_use_case_get(ah->ucm.uc_mgr, "_verb", &new_verb);
        new_dev_count = snd_use_case_get_list(ah->ucm.uc_mgr, "_enadevs", &new_dev_list);
        __dump_use_case(new_verb, new_dev_list, new_dev_count, NULL, 0);
...
audio_return_t _audio_ucm_set_modifiers(audio_hal_t *ah, const char *verb, const char *modifiers[])
{
    ...
    snd_use_case_get(ah->ucm.uc_mgr, "_verb", &old_verb);
    old_mod_count = snd_use_case_get_list(ah->ucm.uc_mgr, "_enamods", &old_mod_list);
    __dump_use_case(old_verb, NULL, 0, old_mod_list, old_mod_count);

    if (modifiers) {
        for (mod_count = 0; modifiers[mod_count]; mod_count++);
    }

    __dump_use_case(verb, NULL, 0, modifiers, mod_count);
...
exit:
    ...
    if (is_verb_changed == 1 || is_mod_changed == 1) {
        ...
        snd_use_case_get(ah->ucm.uc_mgr, "_verb", &new_verb);
        new_mod_count = snd_use_case_get_list(ah->ucm.uc_mgr, "_enamods", &new_mod_list);
        __dump_use_case(new_verb, NULL, 0, new_mod_list, new_mod_count);
